---
title: "Bechdel"
author: "Mine Çetinkaya-Rundel,copied and changed by Rumo Schilling"
format: html
editor: visual
---
...
Zooming in on the movies with `roi < 42`(it was 15 at the beginning but I played around)
provides a better view of how the medians across the categories compare:

```{r}
#| warning: false

ggplot(data = bechdel90_13)+geom_boxplot(mapping = aes(x = clean_test, y = roi, color = binary)) +
  labs(
    title = "Return on investment vs. Bechdel test result",
    subtitle = "42", # Something about zooming in to a certain level
    x = "Detailed Bechdel result",
...
### Playing around
I want to have the median of the profit gained per movie, as I think that profit is more important that roi
```{r}
bechdel90_13 <- bechdel90_13 %>% 
mutate(profit = domgross_2013 + intgross_2013-budget_2013)
ggplot(bechdel90_13) +geom_boxplot(mapping = aes(x = clean_test,y = profit,color = binary))
```
```{r}
bechdel90_13 %>% group_by(binary) %>%
  summarise(
    medprofit = median(profit))
```