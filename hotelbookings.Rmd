...
select(babies,children,hotel)
hotels %>%
filter(hotel == "City Hotel" & children >= 1 | babies >= 1) %>% 
select(babies,children,hotel)

# you see that in city hotels there are less little ones.
# pay attention to correctness and code style
```
...
```{r }
#| label: most common frequency of adults.
hotels %>%
...
count(adults,sort = TRUE) %>%
arrange(is_canceled,adults)
# you see that all bookings over 3 persons got cancelled
# pay attention to correctness and code style
```
...
```{r}
#| label: mean,median,min and max adr
hotels %>% group_by(hotel) %>% 
...
```{r}
#| label: spotting the observations 
hotels %>%
filter(adr<0 |adr >5000)%>% 
mutate(people = adults + children + babies) %>%
select(adr,hotel,people,contains("arrival"))
```
## Data dictionary
